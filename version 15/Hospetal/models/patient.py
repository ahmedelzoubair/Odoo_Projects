...
class HospitalPatients(models.Model):
    ...
    age = fields.Integer(compute='_compute_age', inverse="_inverse_compute_age", search='_search_age',tracking=True)  # note:- the compute filed do not store by default database
    ...
    def _check_date_of_birth(self):  # => (_check_) tare2t ketaba (date_of_birth) el filed 2ele hay2sl 3aleh constrains
        for rec in self:
            if rec.date_of_birth and rec.date_of_birth >= fields.Date.today():  # => lo add date ba3d today date el (_compute_age) func hat7sb el date by -ve value
    ...
    def _compute_appointment_count(self):
        ...
        for app in appointment_group:  # el for de 3shan 2a3adi 3ala koll filed (same of for loop at self)
            ...
            patient_rec = self.browse(patient_id)  # .browse() :- method hada5alk gowa modle, patient_id:- el id 2ele gebto men (.read_group) => 2d5ol 3ala model 2ele 2nta feh get el rec (self) with id = 3
            patient_rec.appointment_count = app.get('patient_id_count')  # after getting all fields of patient with id (3) i will select appointment_count and add in it patient_id_count (9) that created from read_group methode
        ...
        # 2o5rgo bara for loop we 2e self.appointment_count (hospital.patient(17,50)) = 0
