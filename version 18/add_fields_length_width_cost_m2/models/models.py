...
class AddFieldsLengthWidthCostM2Product(models.Model):
    ...
    # final_price = (length * width) * price_per_meter

    @api.onchange('length', 'width', 'price_per_meter')
    def _compute_list_price(self):
        for product in self:
            # Calculate list_price as length * width * price_per_meter
            product.list_price = (product.length or 0.0) * (product.width or 0.0) * (product.price_per_meter or 0.0)
...
class AddFieldsLengthWidthCostM2Variants(models.Model):
    ...
    def _compute_list_price(self):
        for product in self:
            # Calculate list_price as length * width * price_per_meter
            product.lst_price = (product.length or 0.0) * (product.width or 0.0) * (product.price_per_meter or 0.0)
